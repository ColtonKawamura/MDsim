...
# Oscillaitng and plotting. No damping
function cellAcoust()
    mass = 1
    boxX = 1000
    boxY = 5.6896
...
function general()
    ...
    box = Box([boxX,boxY],cutoff)
    k = 100
    # forceLaw = (forceList, particleList, v) -> forces_CL!(k, forceList, particleList, ForceHookeCL, box, cl) # No damping
    gamma = 5
    forceLaw = (forceList, particleList, v) -> forcesDamped!(k, gamma, forceList, particleList, v, ForceHookeDamped, box, cl)
...
function DampedAcou2()
    ...
    cl = CellList([p.position for p in particleList], box)
    # Create the force law with k and gamma pre-defined
    forceLaw = foo(k, gamma)
    VelInitial = VelocitiesInit(particleList, 0, 1)
    trajectory = simAcou(particleList, VelInitial, 1, 0.001, 500, 10, forceLaw, side)
    plotTrajectoryAcoustic(trajectory)
end

function foo(k,gamma)
